...
/// Coordinates tab selection between a [AppTabBar] and a [AppTabScaffold].
///
/// The [index] property is the index of the selected tab. Changing its value
/// updates the actively displayed tab of the [AppTabScaffold] the
/// [AppTabController] controls, as well as the currently selected tab item of
/// its [AppTabBar].
///
/// {@tool snippet}
///
/// [AppTabController] can be used to switch tabs:
///
/// ```dart
...
/// See also:
///
///  * [AppTabScaffold], a tabbed application root layout that can be
...
class AppTabController extends ChangeNotifier {
  /// Creates a [AppTabController] to control the tab index of [AppTabScaffold]
  /// and [AppTabBar].
  ///
  ...
  AppTabController({int initialIndex = 0})
      : _index = initialIndex,
  ...
  /// [AppTabScaffold] controlled by this [AppTabController], as well
  /// as the currently selected tab item of its [AppTabScaffold.tabBar].
  ///
...
/// behind the tab bar.
///
/// A [tabBar] and a [tabBuilder] are required. The [AppTabScaffold]
/// will automatically listen to the provided [AppTabBar]'s tap callbacks
...
/// create its own [AppTabController] and manage it internally. Otherwise
...
/// [AppTabScaffold] may lose some tabs' state in the process.
///
...
/// See also:
///
///  * [AppTabBar], the bottom tab bar inserted in the scaffold.
///  * [AppTabController], the selection state of this widget.
...
class AppTabScaffold extends StatefulWidget {
  ...
  /// The [tabBar] is a [AppTabBar] drawn at the bottom of the screen
  /// that lets the user switch between different tabs in the main content area
  /// when present.
  ///
  /// The [AppTabBar.currentIndex] is only used to initialize a
  /// [AppTabController] when no [controller] is provided. Subsequently
  /// providing a different [AppTabBar.currentIndex] does not affect the
  /// scaffold or the tab bar's active tab index. To programmatically change
  /// the active tab index, use a [AppTabController].
  ///
  /// If [AppTabBar.onTap] is provided, it will still be called.
  /// [AppTabScaffold] automatically also listen to the
  /// [AppTabBar]'s `onTap` to change the [controller]'s `index`
  /// and change the actively displayed tab in [AppTabScaffold]'s own
  /// main content area.
  ///
  ...
  /// items inside a [MediaQuery] with the desired [MediaQueryData.textScaler]
  /// value. The text scale factor value from the operating system can be retrieved
  /// int many ways, such as querying [MediaQuery.textScalerOf] against
  ...
  /// Restoration ID to save and restore the state of the [AppTabScaffold].
  ///
...
class _AppTabScaffoldState extends State<AppTabScaffold> with RestorationMixin {
  RestorableCupertinoTabController? _internalController;
  AppTabController get _controller =>
  ...
  void _updateTabController([AppTabController? oldWidgetController]) {
  ...
  @override
  void didUpdateWidget(AppTabScaffold oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    if (existingMediaQuery.size.width >= 768) {
      contentPadding = contentPadding.copyWith(bottom: 0);
    }
        ...
        tabFocusNodes.addAll(
          List<FocusScopeNode>.generate(
            ...
            (int index) => FocusScopeNode(
                debugLabel:
                    '$AppTabScaffold Tab ${index + tabFocusNodes.length}'),
          ),
        );
      }
    }
...
/// [AppTabController].
///
/// The [AppTabController] is accessible via the [value] getter. During
/// state restoration, the property will restore [AppTabController.index]
/// to the value it had when the restoration data it is getting restored from
/// was collected.
class RestorableCupertinoTabController
    extends RestorableChangeNotifier<AppTabController> {
  ...
  @override
  AppTabController createDefaultValue() {
    return AppTabController(initialIndex: _initialIndex);
  }

  @override
  AppTabController fromPrimitives(Object? data) {
    assert(data != null);
    return AppTabController(initialIndex: data! as int);
  }
